...
class Register(r: CtxSymbol) {
  ...
  def getLong(num: Int): CtxSymbol = Main.simple(reg.select(trans(num) & 0x07))
  ...
  private def setByteHigh(data: CtxSymbol, num: Int): Unit = {
    ...
    reg = reg.store(n, Main.simple(base.extract(31, 16) concat data concat base.extract(7, 0)))
  }

  private def setByteLow(data: CtxSymbol, num: Int): Unit = {
    val n = trans(num) & 0x07
    reg = reg.store(n, Main.simple(reg.select(n).extract(31, 16) concat data))
  }
  ...
  private def setWordHigh(data: CtxSymbol, num: Int) = {
    val n = trans(num) & 0x07
    reg = reg.store(n, Main.simple(data concat reg.select(n).extract(15, 0)))
  }
  ...
  def setLong(data: CtxSymbol, num: Int): Unit = reg = Main.simple(reg.store(trans(num) & 0x07, data))
